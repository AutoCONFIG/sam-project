...
- 后端 `-c` 是 Hydra config 名，相对于 `sam3/sam3/train/`（`initialize_config_module` 限制，配置必须在子模块内）；前端已屏蔽此细节——模型配置的 `hydra:` 段在启动时按文本原样生成到子模块 `configs/_custom/<文件名>.yaml`（自动管理，内容不变不重写，勿手改）；训练配置顶层 `hydra_config` 字段（或 CLI `--sam3-config`）为指向子模块内现成配置（如官方 roboflow 参考配置）的逃生舱
