...
#
# 训练超参数 (transforms/loss/优化器/调度器/评测等全部默认值) 都在训练模板
# configs/train/template_image.yaml —— 包括训练策略相关的两个高级项
# (本 fork 新增的后端支持, 在模板的 trainer 节里):
...
#
# 启动训练时前端把模型配置的 trainer.model 段文本合并进训练模板, 生成到子模块
